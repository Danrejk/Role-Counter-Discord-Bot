...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option
				.setName("ign")
				.setDescription("in-game nickname")
				.setRequired(true)
		)
	...
	execute: async ({ client, interaction }) => {
		...
		if (adminGuild) {
			interviewsChannel =
				adminGuild.channels.cache.get(interviewsChannelId);
			if (!interviewsChannel) {
				await interaction.reply({
					content:
						"There has been an error getting the Interviews channel from the Admin discord",
					ephemeral: true,
				});
			}
			...
			if (user == null) {
				...
				applicant = threadMembers.filter((u) =>
					u.roles.cache.has(applicantRoleId)
				);
				if (applicant.size != 1) {
					// if no definitive applicant is found
					return await interaction.reply({
						content:
							"No applicant or more than one were found. Manual input is needed",
						ephemeral: true,
					});
				}
			...
			const openedThread = await sentInterview.startThread({
				name: username,
			});
			await openedThread.send(
				`<@&${adminRoleIdAdmin}> <@&${modRoleIdAdmin}> <@&${chieftainRoleIdAdmin}>\n${threadLink}\n`
			);
			...
			await interaction.reply(
				`The interview of the user *${user.user} (IGN: ${ign})* has been sent to the Admin Chat!`
			);
		}
	},
};
