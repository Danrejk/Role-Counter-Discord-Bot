...
module.exports = {
	data: new SlashCommandBuilder()
        ...
        .addRoleOption(option => option.setName("role").setDescription("A role to be removed from the watch list.").setRequired(true))
	...
	execute: async ({ client, interaction }) => {
                ...
                db.all(`SELECT roleId FROM watchedRoles WHERE roleId LIKE ?`, [roleId], async(err, rows) => {
                        ...
                        if(result.length == 0){
                                ...
                                db.run(`DELETE FROM watchedRoles WHERE roleId LIKE ?`, [roleId], async(err) => {
                                        if (err) {
