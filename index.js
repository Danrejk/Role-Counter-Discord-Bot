...
client.on("interactionCreate", async (interaction) => {
	...
	try {
		...
		await interaction.reply({
			content:
				"Sorry little one. Something screwed up. <@512664079496642575> please fix it.",
		});
	}
});
...
client.on("guildMemberUpdate", (oldMember, newMember) => {
	...
	const addedRole = [
		...newRoles.filter((role) => !oldRoles.has(role.id)),
	][0]?.[1].name;
	const removedRole = [
		...oldRoles.filter((role) => !newRoles.has(role.id)),
	][0]?.[1].name;

	if (
		roleSymbols.includes(addedRole?.[0]) ||
		roleSymbols.includes(removedRole?.[0])
	) {
...
client.on("roleDelete", (deletedRole) => {
	console.log(
		`The role ${deletedRole.name} has been removed from the server.`
	);
