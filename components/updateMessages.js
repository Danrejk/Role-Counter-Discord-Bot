...
function updateMessages({ client, interactionGuildId }) {
	...
	db.all(
		`SELECT * FROM updatedMessages WHERE guildId = ?`,
		[interactionGuildId],
		async (err, results) => {
			if (err) {
				console.error(err);
			...
			let { countries, city_states, subjects, organisations, religions } =
				await statusMessages({ client, interactionGuildId });
			...
			for (const message of results) {
				try {
					...
					// Update all messages
					fetchedMessage = await channel.messages.fetch(
						message.countriesMessageId
					);
					fetchedMessage.edit(countries);
					fetchedMessage = await channel.messages.fetch(
						message.city_statesMessageId
					);
					fetchedMessage.edit(city_states);
					fetchedMessage = await channel.messages.fetch(
						message.subjectsMessageId
					);
					fetchedMessage.edit(subjects);
					fetchedMessage = await channel.messages.fetch(
						message.organisationsMessageId
					);
					fetchedMessage.edit(organisations);
					fetchedMessage = await channel.messages.fetch(
						message.religionsMessageId
					);
